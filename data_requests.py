...
def get_station_data(station_id: str) -> StationData:
    ...
    slots = [
        Slot(index=slot["index"], empty=slot["empty"], charge=slot["charge"])
        for slot in raw_data.json()["slots"]
    ]
    return StationData(
        id=station_id,
        tariff_id=raw_data.json()["tariff_id"],
...
def get_user_profile(user_id: str) -> UserProfile:
    ...
    return UserProfile(
        id=user_id,
        has_subscribtion=bool(raw_data.json()["has_subscribtion"]),
...
def eject_powerbank(station_id: str) -> EjectResponse:
    ...
    return EjectResponse(
        success=raw_data.json()["success"], powerbank_id=raw_data.json()["powerbank_id"]
    )
